      ...
      <div style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <div className="max-w-7xl mx-auto px-6 py-14 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <h3 className="font-display text-3xl font-light mt-2" style={{ color: 'white' }}>
              Rejoignez notre communauté beauté
            </h3>
          </div>
          <form className="flex w-full md:w-auto gap-0" onSubmit={e => e.preventDefault()}>
            <input type="email" placeholder="Votre adresse e-mail"
                   className="flex-1 md:w-72 px-5 py-3.5 text-sm outline-none"
      ...
      <div className="max-w-7xl mx-auto px-6 py-14 grid grid-cols-2 md:grid-cols-4 gap-10">
        <div className="col-span-2 md:col-span-1">
          ...
          <p className="mt-4 text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.5)' }}>
            Votre destination premium pour la beauté et le bien-être au Congo.
          </p>
          {/* Social icons */}
          <div className="flex gap-4 mt-6">
            <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer"
               className="w-9 h-9 flex items-center justify-center rounded-full transition-all hover:opacity-100"
               style={{ border: '1px solid rgba(255,255,255,0.2)', opacity: 0.6 }}>
            ...
            <a href="#" className="w-9 h-9 flex items-center justify-center rounded-full transition-all hover:opacity-100"
               style={{ border: '1px solid rgba(255,255,255,0.2)', opacity: 0.6 }}>
        ...
        <div>
          ...
          {['Skincare', 'Soins du Corps', 'Protection Solaire', 'Compléments', 'Parfums', 'Capillaire'].map((l, i) => {
            const slugs = ['skincare','body-care','sun-care','supplements','fragrances','hair-care']
            return (
              <Link key={l} href={`/shop?cat=${slugs[i]}`}
                    className="block text-xs mb-3 hover:text-white transition-colors">{l}</Link>
            )
          })}
        </div>
        ...
        <div>
          ...
          {['FAQ', 'Livraison & Retours', 'Suivi de commande', 'Contact'].map(l => (
            <Link key={l} href="#" className="block text-xs mb-3 hover:text-white transition-colors">{l}</Link>
          ))}
          <a href={`https://wa.me/${WA_NUMBER}`} target="_blank" rel="noopener noreferrer"
             className="block text-xs mb-3 hover:text-white transition-colors"
             style={{ color: '#25D366' }}>
            WhatsApp +{WA_NUMBER}
          </a>
        </div>
        ...
        <div>
          ...
          {['Notre Histoire', 'Nos Valeurs', 'Ingrédients', 'Presse'].map(l => (
            <Link key={l} href="#" className="block text-xs mb-3 hover:text-white transition-colors">{l}</Link>
          ))}
          {/* Instagram follow CTA */}
          <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer"
             className="mt-4 flex items-center gap-2 px-4 py-2.5 text-xs font-medium transition-all hover:opacity-100"
             style={{ background: 'linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)', color: 'white', opacity: 0.85 }}>
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col md:flex-row items-center justify-between gap-3">
          <p className="text-[11px]" style={{ color: 'rgba(255,255,255,0.35)' }}>
            © {new Date().getFullYear()} Body & Face. Tous droits réservés.
          </p>
          <div className="flex gap-6">
            {['Mentions légales', 'Politique de confidentialité', 'CGV'].map(l => (
              <Link key={l} href="#" className="text-[11px] hover:text-white transition-colors"
